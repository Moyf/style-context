...
const ja: Messages = {
	...
	settings: {
		...
		descriptions: {
			...
			publishBackgroundImage: '公開済みの画像変数を固定キャンバス背景として表示します。背景レイヤーはクリックを遮らないため、ノートやコントロールに影響しません。',
			backgroundVariable: '上のローカル画像変数の CSS 変数名を入力するか、シャッフルボタンでランダムに選択します。',
		...
		placeholders: { themeClassPrefix: 'Theme-mod-', folderPrefix: 'フォルダの接頭辞', keywordInPath: 'パス内のキーワード', classNames: 'class1, class2', vaultFilePath: '保管庫内のファイルパス', cssVariable: '--my-var', backgroundVariable: '--image-1' },
		buttons: { addPathRule: 'パスのルールを追加', addImageVariable: '画像変数を追加', deleteRule: 'ルールを削除', refresh: '更新', copySnapshot: 'スナップショットをコピー', randomBackgroundVariable: '画像変数をランダムに選択' },
		tooltips: {
