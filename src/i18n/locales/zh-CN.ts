...
const zhCN: Messages = {
	...
	settings: {
		...
		descriptions: {
			...
			themePrefixBefore: '根据当前主题名称添加 body 类，用于按主题设置样式。',
			themePrefixExampleBefore: '例如，使用“Brutal Gum”主题时，会注册类 ',
			themePrefixExampleAfter: '。',
			themeClassCopyHint: '（点击复制）',
			currentThemeClass: '当前主题的 mod CSS 类：',
			publishBackgroundImage: '将 CSS 图片值渲染为固定画布背景。背景层不会拦截鼠标，因此不会影响笔记和控件。',
			perModeBackground: '为亮色模式和暗色模式分别使用不同的图片值与外观。每个窗口根据其 body 上的 theme-light 或 theme-dark 类应用对应配置。',
			randomBackgroundOnStartup: '每次打开 Obsidian 时，从符合条件的图片变量中随机选择一张背景图片。',
			addRandomBackgroundRibbon: '开启后，将随机选择背景图片命令添加到 Ribbon。',
			backgroundFadeDuration: '背景图片出现时淡入，切换时淡出再淡入；0 表示关闭动画。',
			mobileToolbarTransparent: '将移动端工具栏背景设为透明，使画布背景透出。',
			statusBarTransparent: '将状态栏背景设为透明，使画布背景透出。',
			ribbonTransparent: '将 Ribbon 背景设为透明，使画布背景透出。',
			titlebarTransparent: '将窗口标题栏背景设为透明，使画布背景透出。',
			backgroundImageValue: '填写完整的 CSS 图片值，例如 var(--image-1) 或 url("https://example.com/image.jpg")。只写变量名（--image-1）也可以，会自动按 var(--image-1) 处理。远程 URL 会连接图片所在网站。',
			backgroundImageShuffleHint: '随机按钮会选择一个本地图片变量。',
