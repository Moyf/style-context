...
const zhCN: Messages = {
	...
	settings: {
		...
		groups: { themeContext: '主题上下文', notePathRules: '笔记路径规则', localImageVariable: '本地图片变量', backgroundImage: '背景图片', backgroundDisplay: '显示', backgroundFilter: '滤镜', backgroundLayout: '布局', diagnostics: '诊断' },
			pages: { backgroundAppearance: '外观', backgroundAppearanceDesc: '背景图片图层的不透明度、混合、尺寸、位置与 CSS 滤镜。滤镜保持默认值即不生效。', backgroundRandomization: '随机化图像背景', backgroundRandomizationDesc: '设置随机背景图片的执行时机和入口。' },
